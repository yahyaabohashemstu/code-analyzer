...
def visualize_code_graph(G):
    ...
    nx.draw(G, pos, with_labels=True, node_color='skyblue', node_size=1500, edge_color='gray', linewidths=1.5, font_size=10)
    plt.title('Code Graph', size=15)
    plt.show()
...
def visualize():
    ...
    if code and detector:
        graph = detector.code_to_graph(code)
        buf = io.BytesIO()
        plt.figure(figsize=(10, 8))
        pos = nx.spring_layout(graph)
        nx.draw(graph, pos, with_labels=True, node_color='skyblue', node_size=1500, edge_color='gray', linewidths=1.5, font_size=10)
        plt.title('Code Graph', size=15)
        plt.savefig(buf, format='png')
        buf.seek(0)
        chart_url = base64.b64encode(buf.getvalue()).decode('utf-8')
        return render_template_string('<img src="data:image/png;base64,{{ chart_url }}">', chart_url=chart_url)

    return 'Invalid code or language', 400
...
<div class="header">
    <div class="header-content">
        ...
        <form method="POST" action="/visualize">
            ...
            <div class="form-group">
                <button type="submit" class="button">Visualize Code Graph</button>
            </div>
        </form>
    </div>
    <script>
